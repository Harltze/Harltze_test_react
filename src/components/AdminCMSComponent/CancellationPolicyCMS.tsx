...
import { CancellationPolicyInterface, CMSInterface } from '../../pages/adminpages/cms-settings';
...
export default function CancellationPolicyCMS({refreshCMS, cmsData, updateCMSRecord}: Props) {

  const [cancellationPolicy, setCancellationPolicy] = useState<CancellationPolicyInterface[]>([]);
  ...
  const handleEdit = (id: string) => {
    const findValue = cancellationPolicy.find(v => v._id == id);
  ...
  const removeFromList = (id: string) => {
    const value = cancellationPolicy.filter(h => h._id != id);
  ...
  const addOrUpdateButton = () => {
    ...
    if(idToEdit) {
      const value = cancellationPolicy.map((heroData: CancellationPolicyInterface) => {
  ...
  const saveButton = async () => {
    try {
      ...
      const h = cancellationPolicy.map(value => {
      ...
      await updateCMSRecord("cancellation-policy", {cancellationPolicy: h});

      await refreshCMS();

      setAnyUnsavedChanges(false);

      toast.success("Cancellation policy updated succesfully");
  ...
  return (
    <div className='p-4 h-[95%]'>
      ...
      <div className='grid grid-cols-1 gap-4 h-full overflow-y-auto'>
        {
          cancellationPolicy?.map((tacData) => {
            return (
              <div className='flex flex-col gap-4 border border-2 rounded-md p-2 h-fit'>
                ...
                <div>
                  ...
                  <div dangerouslySetInnerHTML={{__html: tacData?.description}} />
                </div>
                  ...
                  <Typography>
                    <div className="flex justify-between items-center mb-8">
                      <div className="font-bold text-[20px]">
                        {idToEdit ? "Update" : "Create New"} Cancellation Policy
                      </div>
